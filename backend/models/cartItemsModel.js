...
export const getCartItems=async(cart_id)=>{
    const [rows]=await db.execute(
        `select ci.id as cart_item_id,ci.quantity,ci.price_at_time,p.id AS product_id,p.name,p.main_image
        FROM cart_items ci
...
export const removeSingleCartItem = async (cart_id, product_id) => {
    await db.execute(
